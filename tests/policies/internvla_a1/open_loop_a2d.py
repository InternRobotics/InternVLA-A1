...
from lerobot.policies.InternVLA_A1_2B import InternVLAA1Policy, InternVLAA1Config
...
def main():
    ...
    # ckpt_path = Path(f"outputs/internvla_a1_2B/2025_12_06_02_53_28-qwenvla-jcaiaq_a2d_pick_pen-delta-28l-scratch_60k-streaming/checkpoints/060000/pretrained_model") # with gen out layernorm
    # ckpt_path = Path(f"/mnt/shared-storage-user/internvla/Users/zengjia/internvla_a1/outputs/internvla_a1_2B/2025_12_06_09_14_02-qwenvla-jcaiaq_a2d_pick_pen-delta-28l-scratch_60k-streaming/checkpoints/060000/pretrained_model") # without gen out layernorm
    # ckpt_path = Path(f"outputs/scratch/internvla_a1_2b/internvla_a1_2b-jcaiaq_a2d_pick_pen-scratch_60k-2025_12_07_02_55_40/checkpoints/060000/pretrained_model")
    # ckpt_path = Path(f"outputs/scratch/internvla_a1_2b/a1_2b-jcaiaq-a2d_pick_pen-pt_355k-ft_60k-12_15_03_08/checkpoints/060000/pretrained_model")
    ckpt_path = Path(f"/home/caijunhao/internvla/Users/zengjia/lerobot_lab/outputs/scratch/internvla_a1_2b/a1_2b-jcaiaq-a2d_pick_pen-pt_355k-ft_60k-12_15_03_08/checkpoints/060000/pretrained_model")
    # ckpt_path = Path(f"outputs/scratch/internvla_a1_2b/internvla_a1_2b-jcaiaq_a2d_pick_pen-scratch_60k-2025_12_07_02_55_40/checkpoints/060000/pretrained_model/")
    ...
    num_total_params_und_expert = sum(p.numel() for n, p in policy.named_parameters() if "und_expert" in n)
    num_total_params_gen_expert = sum(p.numel() for n, p in policy.named_parameters() if "gen_expert" in n)
    num_total_params_act_expert = sum(p.numel() for n, p in policy.named_parameters() if "act_expert" in n)
    print(f"\nTotal parameters: {num_total_params:,}  ({num_total_params / 1e9:.2f}B)")
    print(f"\nUnd expert parameters: {num_total_params_und_expert:,}  ({num_total_params_und_expert / 1e9:.2f}B)")
    print(f"\nGen expert parameters: {num_total_params_gen_expert:,}  ({num_total_params_gen_expert / 1e9:.2f}B)")
    print(f"\nAct expert parameters: {num_total_params_act_expert:,}  ({num_total_params_act_expert / 1e9:.2f}B)")
    ...
    output_dir = Path(f"outputs/eval/internvla_a1_2B/{ckpt_name}/open_loop_results/last/{cfg.dataset.repo_id}")
    ...
    for ep_id in range(min(num_episodes, 10)):
        ...
        for idx in range(from_ids[ep_id], to_ids[ep_id], config.chunk_size):
            ...
            with torch.no_grad():
                ...
                'action': (1, 50, 32)
                'observation.images.image0': (1, 3, 3, 448, 448)
                'observation.images.image1': (1, 3, 3, 448, 448)
                'observation.images.image2': (1, 3, 3, 448, 448)
                'observation.images.image0_mask': (1,)
                'observation.images.image1_mask': (1,)
                'observation.images.image2_mask': (1,)
        ...
        plt.savefig(output_dir / "plots" / f"a1_2b_open_loop_ep{ep_id}.jpg")
    log = {
        "MSE": metric_mse, 
